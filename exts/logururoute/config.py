...
def setup_ext_loguru(log_pro_path: str = None):
    ...
    LOGURU_FORMAT: str = (
        "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green>  | thread_id:{thread.id} thread_name:{thread.name} | {level} | {message}"
    )
    # 这句话很关键避免多次的写入我们的日志
    ...
    # logger.remove()
    # 错误日志不需要压缩
    format = "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green>  | thread_id:{thread.id} thread_name:{thread.name} | {level} | {message}"
